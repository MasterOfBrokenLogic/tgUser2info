...
:root{
  ...
  --yellow:#ffcc00;--grid:rgba(0,255,136,0.03);
}
...
.lcard{
  ...
  border:1px solid var(--border);padding:36px 28px;position:relative;
...
.linput{
  width:100%;background:rgba(0,0,0,0.5);border:1px solid var(--dim);color:#fff;
  font-family:'Share Tech Mono',monospace;font-size:14px;letter-spacing:2px;
  padding:12px 14px;outline:none;transition:border-color 0.2s;margin-bottom:16px;
}
.linput:focus{border-color:var(--green);}
.lbtn{
  width:100%;padding:14px;background:transparent;border:1px solid var(--green);color:var(--green);
  font-family:'Bebas Neue',sans-serif;font-size:17px;letter-spacing:5px;cursor:pointer;
...
.search-inp{
  background:rgba(0,0,0,0.4);border:1px solid var(--dim);color:#fff;
  font-family:'Share Tech Mono',monospace;font-size:11px;letter-spacing:1px;
  padding:7px 12px;outline:none;transition:border-color 0.2s;width:200px;
}
...
.bar{width:100%;min-height:2px;border-radius:1px 1px 0 0;transition:opacity 0.2s;opacity:0.7;}
.bar:hover{opacity:1;}
...
.kbtn{
  ...
  padding:5px 10px;cursor:pointer;background:none;border:1px solid;transition:all 0.15s;
}
...
.no-keys{padding:32px;text-align:center;font-family:'Share Tech Mono',monospace;font-size:10px;letter-spacing:3px;color:var(--muted);}
...
.modal{
  background:var(--surface);border:1px solid var(--border);max-width:440px;width:100%;
...
.minput{
  width:100%;background:rgba(0,0,0,0.4);border:1px solid var(--dim);color:#fff;
  font-family:'Share Tech Mono',monospace;font-size:14px;letter-spacing:2px;
...
<div id="loginWrap">
  <div class="lcard">
    <div class="llogo">4N<span>SIL</span></div>
...
<div id="dash">
  <header>
    <div style="display:flex;align-items:center;gap:14px;">
      <div class="hlogo">4N<span>SIL</span></div>
      <div class="hbadge">ADMIN</div>
    </div>
    <div class="hbtns">
      <button class="hbtn green" onclick="openAdd()">+ NEW KEY</button>
      <button class="hbtn green" onclick="load()">REFRESH</button>
    ...
    <div class="sec">
      <div class="sec-hdr">
        ...
        <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
          <input class="search-inp" type="text" id="searchInp" placeholder="SEARCH KEY..." oninput="filterKeys(this.value)"/>
          <span style="font-family:'Share Tech Mono',monospace;font-size:9px;color:var(--muted);letter-spacing:2px;">
...
function filterKeys(q) {
  ...
  _filtered = v ? Object.entries(_data).filter(([k,d])=>k.toLowerCase().includes(v)||(d.label||"").toLowerCase().includes(v)) : null;
  renderKeys();
}
...
function renderKeys() {
  ...
  if (!entries.length) {
    grid.innerHTML = '<div class="no-keys">NO KEYS FOUND</div>'; return;
  }

  grid.innerHTML = entries.map(([ks, k]) => {
    const todayN  = (k.dailyUsage||{})[TODAY]||0;
    const lastUsed= k.lastUsed ? timeSince(k.lastUsed) : "NEVER";
    const created = k.createdAt ? fmtDate(k.createdAt) : "—";
    const on      = k.active;

    const blockBtn = on
      ? \`<button class="kbtn block" data-a="block" data-k="\${ks}">BLOCK</button>\`
      : \`<button class="kbtn unblock" data-a="unblock" data-k="\${ks}">UNBLOCK</button>\`;

    return \`<div class="kcard\${on?'':' off'}">
      <div class="ktop">
        <span class="kname">\${k.label||ks}</span>
        <span class="kbadge \${on?'on':'off'}">\${on?'ACTIVE':'BLOCKED'}</span>
      </div>
      <div class="krows">
        ...
        <div class="kr"><span class="krl">LAST USED</span><span class="krv">\${lastUsed}</span></div>
        <div class="kr"><span class="krl">CREATED</span><span class="krv">\${created}</span></div>
      </div>
      \${miniChart(k.dailyUsage||{})}
      <div class="kacts">
  ...
  grid.onclick = e => {
    ...
    if (a==="block")   confirm2(\`BLOCK KEY: \${k}\`,   \`Block "\${k}"? Requests with this key will be rejected.\`, ()=>toggle(k,false));
...
function miniChart(usage) {
  const days = [];
  for (let i=6;i>=0;i--) {
    const d = new Date(); d.setDate(d.getDate()-i);
    const dk = d.toISOString().slice(0,10);
    days.push({ lbl: d.toLocaleDateString("en-IN",{weekday:"short"}).slice(0,2).toUpperCase(), n: usage[dk]||0, today: dk===TODAY });
  }
  const max = Math.max(...days.map(d=>d.n), 1);
  return \`<div class="mchart"><div class="mc-title">LAST 7 DAYS</div><div class="bars">\${
    days.map(d=>{
      const h = Math.max(2, Math.round((d.n/max)*36));
      return \`<div class="bwrap" title="\${d.n} requests"><div class="bar" style="height:\${h}px;background:\${d.today?'var(--cyan)':'var(--green)'}"></div><span class="blbl">\${d.lbl}</span></div>\`;
    }).join("")
  }</div></div>\`;
}
...
async function addKey() {
  const ks = document.getElementById("nkStr").value.trim();
  const lb = document.getElementById("nkLabel").value.trim();
  ...
  closeModal("addModal");
  document.getElementById("nkStr").value = "";
  document.getElementById("nkLabel").value = "";
  errEl.style.display = "none";
  load();
}

async function toggle(ks, active) { await api("PATCH",{key:ks,active}); load(); }
async function del(ks)            { await api("DELETE",null,\`&key=\${encodeURIComponent(ks)}\`); load(); }

function copyKey(ks, btn) {
  navigator.clipboard.writeText(ks).then(()=>{
    const o=btn.textContent; btn.textContent="COPIED!"; btn.style.color="var(--green)";
...
function openAdd() {
  ...
  setTimeout(()=>document.getElementById("nkStr").focus(),80);
}
...
function confirm2(title, body, onOk) {
  ...
  document.getElementById("confOk").onclick = ()=>{ closeModal("confModal"); onOk(); };
...
document.addEventListener("keydown",e=>{
  ...
  if(e.key==="Enter"&&document.getElementById("addModal").classList.contains("open")) addKey();
});
...
setInterval(()=>{ if(document.getElementById("dash").style.display!=="none") load(); },30000);
</script>
</body>
</html>
`;
